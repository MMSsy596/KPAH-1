    ...
    public void update() {
        ...
        for(int j = 0; j < this.monsters.size(); ++j) {
            try {
                ...
                e.printStackTrace();
                Logger.logError(e, "logs/runtime/rUpdateleaf.txt");
            }
        }
    ...
    protected void doAttackPlayer(Char p, Message message) {
        try {
            ...
            if (c.hp > 0 && damNguyetAnh > 0) {
                final int nguyetAnhDamage = c.clampIncomingDamageForHoangCt(c.maxhp * damNguyetAnh / 100);
                c.hp -= nguyetAnhDamage;
                damage += nguyetAnhDamage;
