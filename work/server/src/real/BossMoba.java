...
public class BossMoba extends Boss {
    int team = -1;
    ...
    public BossMoba(Map mapLiveIn, MonsterTemplate template, int x, int y, int country) {
    ...
    public void moveOld() {
        if (this.target != null) {
            ...
            int xx = 6;
            int yy = 6;
            int[] dxy = UtilKPAH.getDXDy(UtilKPAH.angle(this.xTarget - this.x, this.ytarGet - this.y), sp);
            if (UtilKPAH.distance(this.x, this.y, this.xTarget, this.ytarGet) > 12) {
                this.x += dxy[0];
                this.y += dxy[1];
            }
        }
    ...
    public void attack() {
        if (this.getTeam() == 0) {
            RegionMapMoba rg = (RegionMapMoba)MapChienTruongMoba.ALL_REGION.get(this.region);
            long now = System.currentTimeMillis();
            if (now - this.lastTimeAttack > this.attackDelay) {
                this.lastTimeAttack = now;
                new Vector();
                Vector<Char> var25 = rg.getAllPlayer(0);

                for(int i = 0; i < var25.size(); ++i) {
                    Char player = (Char)var25.get(i);

                    try {
                        Message msg = MessageCreator.createMsgNewEffectSkill5Long(EffectBuff.EFF_NO_THUNG_GO, 5000, rg.boss2.id, rg.boss2.cat, true, false);
        ...
        } else if (this.target == null || this.target.map.equals(this.map) && this.target.isMonsterMoba() && (this.target.region == this.region || !this.randomMap) && (this.target.inCountry == this.inCountry || !this.randomMap)) {
            long now = System.currentTimeMillis();
            if (now - this.lastTimeAttack > this.attackDelay) {
                this.lastTimeAttack = now;
            ...
            if (this.isDead) {
                ...
                if (System.currentTimeMillis() > this.bornTime) {
                    ...
                    if (this.idTemplate == 113) {
                        idmap = new int[]{8, 10};
                        xy = new int[][]{{40, 40}, {49, 17}};
                    }

                    if (this.idTemplate == 115) {
                        idmap = new int[]{5, 24, 14};
                        xy = new int[][]{{47, 20}, {22, 40}, {40, 74}};
                    }
                    ...
                    try {
                        Message m = MessageCreator.createServerAlertAutoOffMessage(Map.getNameBossApear(this.inCountry) + " đã xuất hiện.");
    ...
    public void update() {
        ...
        if (this.isDead) {
            this.hp = this.maxhp;
            if (!this.randomMap && System.currentTimeMillis() > this.bornTime && !Char.isFinishAllSuKienHe2015()) {
        ...
        } else if (UtilKPAH.distance(this.x, this.y, this.xTarget, this.ytarGet) <= 12) {
            this.attack();
        } else {
    ...
    public void setTimeReBornInEvent(long time) {
        if (!this.randomMap) {
            this.bornTime = System.currentTimeMillis() + 14400000L;
        } else if (this.idTemplate != 113) {
            Map.event.timeBossAppear[1] = time;
        } else {
            Map.event.timeBossAppear[5] = time;
        }
