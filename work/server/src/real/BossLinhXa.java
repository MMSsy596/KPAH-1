...
public class BossLinhXa extends Boss {
    ...
    public BossLinhXa(Map mapLiveIn, MonsterTemplate template, int x, int y, int country) {
    ...
    public int attackDam(LiveActor actor) {
        ...
        int dam = this.attack;
        if (this.checkSuyYeu()) {
            EffectBuff ef = (EffectBuff)this.hashEffBuff.get(EffectBuff.SUY_YEU);
            dam -= dam * ef.dam / 100;
        }
                    ...
                    for(int i = 0; i < charid.size(); ++i) {
                        ...
                        if (c.hp <= 0) {
                            ...
                            try {
                                ...
                                c.lvDetail.setExp(xp, c.oldLv, c.getName(), "bosslinhxa");
                                if (c.lvDetail.lv <= 1) {
                                    c.lvDetail.lv = 1;
            ...
            if (this.isDead) {
                ...
                if (System.currentTimeMillis() > this.bornTime) {
                    ...
                    try {
                        ...
                        this.map.removeMonster(this.id, this.inCountry, this.region);
                        this.inCountry = EventGame.COUNTRY_BOSS_APPEAR[0];
                        this.map.removeMonster(this.id, this.inCountry, this.region);
