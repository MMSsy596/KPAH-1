...
import server.ClientIntegrityAuth;
...
public class LoginHandler implements ICommandHandler {
    ...
    public static Message createMsgLogin(final int vsTile) {
        ...
        try {
            ...
            String[][] decript3 = {{"Dùng sức mạnh của sấm sét tấn công nhiều mục tiêu trong 1 phạm vi nhất định", " Dùng vũ khí làm cầu nối dẫn truyền sấm sét vào cơ thể đối thủ, tia sét sẽ di chuyển thành một chuỗi liên hoàn vào các đối tượng tập trung gần nhau.", "Vận kình lực tạo thành những thanh kiếm khổng lồ giáng từ trên trời xuống đối phương"}, {"Dùng sức mạnh của rồng lửa tấn công nhiều mục tiêu trong 1 phạm vi nhất định", "Vận nội kình tạo ra liệt hỏa phát tán ra xung quanh, đốt cháy bất kỳ đối tượng nào trên đường đi của liệt hỏa.", " Khuấy động bầu trời, tạo ra những cơn mưa sao băng trút xuống mặt đất"}, {"Dùng sức mạnh của rồng nước tấn công nhiều mục tiêu trong 1 phạm vi nhất định", " Vận nội kình vào hai tay đánh ra hai con rồng theo hình trôn ốc, tạo nên một lực công phá khủng khiếp.", " Gọi gió hô mưa rồi truyền vào đó một luồng hàn băng cực mạnh tạo nên những trận mưa băng khủng khiếp"}, {"Tạo ra những cơn địa chấn tấn công nhiều mục tiêu trong phạm vi nhất định", "Dùng búa tạo ra những trận mưa đá tảng tấn công tới tấp vào đối thủ.", "Giáng một búa mạnh xuống đất tạo nên những cột thạch nhũ lao lên khỏi mặt đất công kích đối phương"}, {"Tẩm cực độc vào mũi tên và triệt hạ nhiều mục tiêu cùng lúc", "Hất tung đối phương lên không trung và bắn liên hoàn năm mũi tên vào đối thủ dẫn đến tử vong.", "Mo ta skill moi"}};
    ...
    public void process(final Session session, final Message message) throws IOException {
        ...
        int userID = 0;
        final ClientIntegrityAuth.VerificationResult clientAuth = ClientIntegrityAuth.verify(platf);
        // by pass dùng hàm login này
        if (TeamServer.isServerIndo() || true) {
            userID = Database.instance.checkLoginIndo(uname.toLowerCase(), pwd);
            if (userID > 0 && !clientAuth.valid) {
                handleClientIntegrityViolation(session, userID, uname, clientAuth, platf);
                return;
            }
        ...
        if (bigProvider.equals("0")) {
            LogInController.Instance.OnMessageLogInComming(session, uname, pwd);
            return;
        }
        ...
        if (userID == -5) {
            ...
            if (!TeamServer.isServerIndo()) {
                ...
                i.dos.writeUTF("Akun Anda sedang menunggu persetujuan admin.");
            }
            session.sendMessage(i);
            i.cleanup();
            message.cleanup();
        ...
        try {
            if (Map.openLog) {
                ...
                // External legacy activity logging is disabled in the local build.
            }
        } catch (final Exception ex7) {
        }
    }

    private static void handleClientIntegrityViolation(
            final Session session,
            final int userID,
            final String uname,
            final ClientIntegrityAuth.VerificationResult result,
            final String platformRaw
    ) {
        boolean locked = false;
        if (result.shouldLockAccount && TeamServer.clientAuthAutoLock) {
            locked = Database.instance.lockTeamUserAccount(uname.toLowerCase(), result.reasonKey);
            if (locked) {
                kickOnlineCharacterForLockedAccount(userID);
            }
        }
        saveClientIntegrityLog(uname, session.ip, result, platformRaw, locked);
        try {
            final Message m = new Message(2);
            if (result.shouldLockAccount && TeamServer.clientAuthAutoLock) {
                m.dos.writeUTF("Client dang nhap khong hop le hoac da bi chinh sua. Tai khoan da bi khoa.");
            } else {
                m.dos.writeUTF(result.message);
            }
            session.sendMessage(m);
            m.cleanup();
        ...
        System.out.println(
                "[client-auth] Reject login user=" + uname
                        + ", ip=" + session.ip
                        + ", reason=" + result.reasonKey
                        + ", clientId=" + result.clientId
                        + ", hash=" + result.clientHash
                        + ", locked=" + locked
                        + ", platform=" + trimClientAuthLogValue(platformRaw, 180)
        );
    }

    private static void kickOnlineCharacterForLockedAccount(final int userID) {
        if (userID <= 0) {
            return;
        }
        final Char online = CharManager.instance.getByUserID(userID);
        if (online == null) {
            return;
        }
        try {
            online.sendMessage(MessageCreator.createServerAlertMessage(
                    "Tai khoan cua ban da bi khoa do dung client khong hop le.",
                    ""
            ));
        } catch (final Exception ignored) {
        }
        try {
            if (online.getSession() != null) {
                online.getSession().disconnect(21);
            }
        } catch (final Exception ignored) {
        }
        try {
            CharManager.instance.kickPlayer(online, "clientauth-lock");
        } catch (final Exception ignored) {
        }
    }

    private static void saveClientIntegrityLog(
            final String uname,
            final String ip,
            final ClientIntegrityAuth.VerificationResult result,
            final String platformRaw,
        ...
        try {
            ...
            info.append("user=").append(sanitizeClientAuthLogValue(uname));
            info.append("|ip=").append(sanitizeClientAuthLogValue(ip));
            info.append("|reason=").append(sanitizeClientAuthLogValue(result.reasonKey));
            if (result.clientId != null && result.clientId.length() > 0) {
                info.append("|clientid=").append(sanitizeClientAuthLogValue(result.clientId));
            }
            if (result.clientHash != null && result.clientHash.length() > 0) {
                info.append("|hash=").append(sanitizeClientAuthLogValue(result.clientHash));
            }
            info.append("|locked=").append(locked ? "1" : "0");
            String platformValue = sanitizeClientAuthLogValue(trimClientAuthLogValue(platformRaw, 180));
            if (platformValue.length() > 0) {
                info.append("|platform=").append(platformValue);
            }
            Database.instance.saveOrtherLog("", uname.toLowerCase(), info.toString(), "clientauth");
        } catch (final Exception ignored) {
        }
    }

    private static String trimClientAuthLogValue(final String value, final int maxLength) {
        if (value == null) {
            return "";
        }
    ...
    private static String sanitizeClientAuthLogValue(final String value) {
        if (value == null) {
            return "";
        }
        return value
        ...
        try {
            ...
            if (userID == -5) {
                ...
                if (!TeamServer.isServerIndo()) {
                    ...
                    m.dos.writeUTF("Akun Anda sedang menunggu persetujuan admin.");
                }
                s.sendMessage(m);
                m.cleanup();
                return;
    ...
    public void doLoginByAdmin(final String charname, final Session session) {
        try {
            ...
            try {
                ...
                final String infoBoss = Map.getNameBossApear(p.inCountry);
                if (!infoBoss.equals("")) {
                    session.sendMessage(MessageCreator.createServerAlertAutoOffMessage(String.valueOf(infoBoss) + " đã xuất hiện"));
                }
