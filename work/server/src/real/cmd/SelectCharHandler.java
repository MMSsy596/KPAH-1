...
//            }
            ...
            if (type == 2) {
                if (p.using == 1) {
                    ...
                    session.sendMessage(MessageCreator.createServerAlertMessage("Nhan vat se bi xoa sau 3 ngay. Co the khoi phuc trong thoi gian nay.", ""));
                } else {
                    session.sendMessage(MessageCreator.createServerAlertMessage("Nhan vat dang trong thoi gian cho xoa.", ""));
                }
                session.sendMessage(MessageCreator.createCharListMessage(session.userID, session));
                return;
            }
            if (type == 3) {
                if (p.using == 0) {
                    Database.instance.updateDelChar(charDBID, 1);
                    session.sendMessage(MessageCreator.createServerAlertMessage("Nhan vat da duoc khoi phuc.", ""));
                } else {
                    session.sendMessage(MessageCreator.createServerAlertMessage("Nhan vat nay khong o trang thai cho xoa.", ""));
                }
                session.sendMessage(MessageCreator.createCharListMessage(session.userID, session));
                return;
            }
            ...
            p.calculatorHPMP();
            final boolean grantedLoginCompensation = p.applyLoginCompensationIfNeeded();
            ...
            if (grantedLoginCompensation) {
                p.sendMessage(MessageCreator.createServerAlertAutoOffMessage("+100 luong den bu. Moi nhan vat chi nhan 1 lan duy nhat."));
            }
            try {
