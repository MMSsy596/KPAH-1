...
public class FruitTemplate {
    ...
    public String getInfoGift() {
        ...
        if (this.getTemplate().type == 1) {
            ...
            if (TeamServer.ExpQua) {
                info = info + ((this.gift > 0) ? (this.gift + "exp") : "exp");
            } else {
    ...
    private int getExpGiftIndex(int lvChar) {
        int index = (lvChar - 34) / 5;
        if (index < 0) {
            index = 0;
        }
        if (index >= EXP.length) {
        ...
        while (index > 0 && EXP[index].length < 2) {
            --index;
        }
        return index;
    }

    private void ensureExpGift(int lvChar) {
        if (this.type != TYPE_EXP || this.gift > 0) {
            return;
        }
        int index = this.getExpGiftIndex(lvChar);
        if (EXP[index].length < 2) {
            this.gift = 0;
            return;
        }
        int min = EXP[index][0];
        ...
        this.gift = min + Map.r.nextInt(max - min);
    }
    ...
    public void addGift(Char p) {
        ...
        switch (this.type) {
            case 0:
                if (TeamServer.ExpQua) {
                    this.ensureExpGift(p.lvDetail.lv);
