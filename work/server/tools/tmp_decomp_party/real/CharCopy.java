...
extends Char {
    ...
    public String getName() {
        ...
        return this.isThangBe() ? (CharCopy.isSuKienMiniChucNu() ? "chuc nu" : "dua be") : this.charname;
    }
        ...
        if (this.follow) {
            ...
            if (this.hp > 0 && this.moved && (this.timedie == 0L || System.currentTimeMillis() - this.timedie > 0L && this.timedie > 0L)) {
                ...
                if (!this.owner.isAdmin) {
                    this.owner.sendToNearPlayer(this.writeActorPos(new Message(4), (Actor)((Object)this)));
                }
            }
            ...
            if (this.hp > 0 && System.currentTimeMillis() - this.timeRegentHpMp > 0L) {
                try {
                    if (this.hp < this.maxhp) {
                        ...
                        this.owner.sendToNearPlayer(MessageCreator.createNew_HMP_Message((Char)this, (int)(this.maxhp / 10)));
                    }
                }
                        ...
                        if (getXp > 0) {
                            ...
                            if ((totalXp = dxp) > 0) {
                                ...
                                if (newxp != totalXp) {
                                    ...
                                    int maxLv = this.owner.lvDetail.lv;
                                    int i = 0;
                                    while (i < this.owner.party.userParty.size()) {
                                        Char pp = this.owner.party.userParty.get(i);
                                        if (pp.id != this.owner.id && this.owner.near((Actor)pp, 320) && pp.mapID == this.owner.mapID && pp.inCountry == this.owner.inCountry && pp.region == this.owner.region) {
                                            int dlv = Map.abs((int)(maxLv - pp.lvDetail.lv));
                                            int temp = 1;
                                            temp = dlv <= 5 ? xpReceive : (dlv <= 10 ? xpReceive / 5 : (dlv <= 20 ? xpReceive / 10 : (dlv <= 30 ? xpReceive / 15 : xpReceive / 20)));
                                            ...
                                            if (pp.hp > 0) {
                                                temp *= Map.doubleALL;
                                                temp = pp.expReceive(temp);
                                                Map.addXPForChar((Char)pp, (long)(temp + pp.getEffSkillClan(0) * temp / 100), (boolean)false, (String)"charcopy4");
                                            }
                                        }
                                        ++i;
                                    }
                                    xpReceive = newxp * 20 / 100 * Map.doubleALL;
                                    xpReceive = this.owner.expReceive(xpReceive);
                                    Map.addXPForChar((Char)this.owner, (long)(xpReceive + this.owner.getEffSkillClan(0) * xpReceive / 100), (boolean)false, (String)"charcopy5");
                                } else {
                                    totalXp *= Map.doubleALL;
                                    totalXp = this.owner.expReceive(totalXp);
                                    Map.addXPForChar((Char)this.owner, (long)(totalXp + this.owner.getEffSkillClan(0) * totalXp / 100), (boolean)false, (String)"charcopy6");
                                }
                            }
                        }
                    ...
                    try {
                        ...
                        if ((totalXp = dxp) > 0) {
                            ...
                            if (newxp != totalXp) {
                                ...
                                while (i < this.owner.party.userParty.size()) {
                                    ...
                                    if (pp.id != this.owner.id && this.owner.near((Actor)pp, 320) && pp.mapID == this.owner.mapID && pp.inCountry == this.owner.inCountry && pp.region == this.owner.region) {
                                        ...
                                        if (pp.hp > 0) {
                                            temp *= Map.doubleALL;
                                            temp = pp.expReceive(temp);
                                            Map.addXPForChar((Char)pp, (long)(temp + pp.getEffSkillClan(0) * temp / 100), (boolean)false, (String)"charcopy7");
                                        }
                                    }
                                    ++i;
                                }
                                xpReceive = newxp * 20 / 100 * Map.doubleALL;
                                xpReceive = this.owner.expReceive(xpReceive);
                                Map.addXPForChar((Char)this.owner, (long)(xpReceive + this.owner.getEffSkillClan(0) * xpReceive / 100), (boolean)false, (String)"charcopy8");
                            } else {
                                totalXp *= Map.doubleALL;
                                totalXp = this.owner.expReceive(totalXp);
                                Map.addXPForChar((Char)this.owner, (long)(totalXp + this.owner.getEffSkillClan(0) * totalXp / 100), (boolean)false, (String)"charcopy9");
                            }
                        }
                    }
                    catch (Exception dxp) {
    ...
    public void setMaxHp() {
        int index = (this.lastLV - 40) / 10;
        this.hp = this.maxhp = HP[index][this.lvLinhthue];
    }
    ...
    public void actorDie() {
        try {
            ...
            this.map.sendAllPlayer(msg, (int)idcountry);
    ...
    public boolean isMyHoVe(Char p) {
        ...
        if (this.isThangBe() || this.owner == null) {
            return true;
        }
