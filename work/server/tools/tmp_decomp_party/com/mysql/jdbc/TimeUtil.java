...
public class TimeUtil {
    ...
    static final Time fastTimeCreate(Calendar cal, int hour, int minute, int second) throws SQLException {
        if (hour < 0 || hour > 23) {
            throw SQLError.createSQLException("Illegal hour value '" + hour + "' for java.sql.Time type in value '" + TimeUtil.timeFormattedString(hour, minute, second) + ".", "S1009");
        }
        if (minute < 0 || minute > 59) {
            throw SQLError.createSQLException("Illegal minute value '" + minute + "'" + "' for java.sql.Time type in value '" + TimeUtil.timeFormattedString(hour, minute, second) + ".", "S1009");
        }
        if (second < 0 || second > 59) {
            throw SQLError.createSQLException("Illegal minute value '" + second + "'" + "' for java.sql.Time type in value '" + TimeUtil.timeFormattedString(hour, minute, second) + ".", "S1009");
        }
        cal.clear();
    ...
    public static String getCanoncialTimezone(String timezoneStr) {
        String[] abbreviatedTimezone;
        String canonicalTz;
        if (timezoneStr == null) {
            return null;
        }
        int daylightIndex = StringUtils.indexOfIgnoreCase(timezoneStr = timezoneStr.trim(), "DAYLIGHT");
        if (daylightIndex != -1) {
        ...
        if ((canonicalTz = (String)TIMEZONE_MAPPINGS.get(timezoneStr)) == null && (abbreviatedTimezone = (String[])ABBREVIATED_TIMEZONES.get(timezoneStr)) != null) {
            if (abbreviatedTimezone.length == 1) {
                canonicalTz = abbreviatedTimezone[0];
            } else {
                StringBuffer errorMsg = new StringBuffer("The server timezone value '");
                errorMsg.append(timezoneStr);
                errorMsg.append("' represents more than one timezone. You must ");
                errorMsg.append("configure either the server or client to use a ");
                errorMsg.append("more specifc timezone value if you want to enable ");
