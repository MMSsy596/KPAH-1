...
public class BossMatQuy extends Boss {
                    ...
                    for(int i = 0; i < charid.size(); ++i) {
                        ...
                        if (c.hp <= 0) {
                            ...
                            try {
                                ...
                                c.lvDetail.setExp(xp, c.oldLv, c.getName(), "bossmatquy");
                                if (c.lvDetail.lv <= 1) {
                                    c.lvDetail.lv = 1;
    ...
    public void actorDie() {
        try {
            try {
                ...
                Map.event.timeBossAppear[0] = this.bornTime;
    ...
    public Vector<GemItem> dropListGemItem() {
        ...
        // Add guaranteed drop of item 268
        GemItem guaranteedItem = new GemItem(268);
        guaranteedItem.isBoss = true; // Set as boss item like other boss drops
        guaranteedItem.cat = 6; // Set category like other boss drops
        gem.add(guaranteedItem);
    
        // Original random drops
    ...
    public Vector<Potion> dropPotion(Char p) {
        ...
        if (Map.r.nextInt(2) == 0) {
            pt.add(new Potion((short)112, 1, this.map));
        } else {
            pt.add(new Potion((short)113, 1, this.map));
        }
