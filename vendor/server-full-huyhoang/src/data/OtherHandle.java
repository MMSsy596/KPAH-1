    ...
    public void process(Session session, Message message) throws IOException {
        switch (message.command) {
            ...
            case -31:
                ...
                String infoalert = "Đăng ký thành công. Dưới 18 tuổi chỉ được chơi 3h mỗi ngày";
                if (idActor1 == 0) {
                    if (text1.equals("")) {
                        infoalert = "Họ và tên không hợp lệ";
                        session.sendMessage(MessageCreator.createServerAlertMessage(infoalert, ""));
                        return;
                    }

                    session.hoten = text1;
                    session.sendMessage(MessageCreator.createMsgInputText(1, 0, "Ngày sinh(dd mm yyyy)", 0));
                } else if (idActor1 == 1) {
                    if (text1.equals("")) {
                        infoalert = "Ngày sinh không hợp lệ";
                        session.sendMessage(MessageCreator.createServerAlertMessage(infoalert, ""));
                        return;
                    }
                    ...
                    if (isDuoi12) {
                        infoalert = "Game chỉ dành cho người chơi từ 12 tuổi trở lên";
                        session.sendMessage(MessageCreator.createServerAlertMessage(infoalert, ""));
                        return;
                    }

                    session.ngaysinh = text1;
                    session.sendMessage(MessageCreator.createMsgInputText(2, 0, "Địa chỉ", 0));
                } else if (idActor1 == 2) {
                    if (text1.equals("")) {
                        infoalert = "Địa chỉ không hợp lệ";
                        session.sendMessage(MessageCreator.createServerAlertMessage(infoalert, ""));
                        return;
                    }
                    ...
                    if (text1.equals("")) {
                        infoalert = "Cmnd không hợp lệ";
                        session.sendMessage(MessageCreator.createServerAlertMessage(infoalert, ""));
                        return;
                    }

                    session.cmnd = text1;
                    session.sendMessage(MessageCreator.createMsgInputText(4, 0, "Ngày cấp", 0));
                } else if (idActor1 == 4) {
                    if (text1.equals("")) {
                        infoalert = "Ngày cấp không hợp lệ";
                        session.sendMessage(MessageCreator.createServerAlertMessage(infoalert, ""));
                        return;
                    }

                    session.ngaycap = text1;
                    session.sendMessage(MessageCreator.createMsgInputText(5, 0, "Nơi cấp", 0));
                } else if (idActor1 == 5) {
                    if (text1.equals("")) {
                        infoalert = "nơi cấp không hợp lệ";
                        session.sendMessage(MessageCreator.createServerAlertMessage(infoalert, ""));
                        return;
                    }
                    ...
                    Database.instance.addInfoAcount(session.usernameReg, info, session.ngaysinh, 0);
                    session.sendMessage(MessageCreator.createServerAlertMessage("Để tạo tài khoản soạn tin " + session.syntax + " " + session.usernameReg + " gửi 6020", ""));
                }
                break;
            case -27:
    ...
    public static void loadImgHorse() {
        ...
        try {
            String[] urlHeader = new String[]{
                ...
                "map/ph/phtho/phuonghoang_data",// đương khang băng
            };
            ...
            for (int i = 0; i < MAX_ANIMAL; ++i) {
                ...
                String[] urlData = new String[]{
                    "map/horse/ngua", // còn này có 3 ngựa
                    "map/trau/ngua",
                    "map/cop/ngua",
                    "map/soi/ngua",
                    "map/hac/ngua",
                    ...
                    "map/ph/phtho/phuonghoang",//lợn băng
                };
