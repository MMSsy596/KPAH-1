...
public class RealController extends ServerController {
    ...
    private static final int DEFAULT_MULTI_REGION_MAP_COUNT = 3;
    public IDGen idGen;
    ...
    public static volatile boolean maintenancePreparing;
    ...
    public void initMapKPAH() {
        ...
        all_map_train.add(m);
        int nregion = DEFAULT_MULTI_REGION_MAP_COUNT;
        ...
        m.setPostRegion(new short[]{38, 55});
        all_map_train.add(m);
        mapList.put(new Integer(20), m = new Map(20, -1, 1, 20, nregion));
    ...
    public void initData() {
        boolean previousSaving = savingChar;
        savingChar = true;

        try {
            this.initMapKPAH();
            this.stabilizeRegionRuntimeData();
        } finally {
            savingChar = previousSaving;
        }
        ...
        commandHandler.put(-40, otherHandle);
        commandHandler.put(-32, otherHandle);
        commandHandler.put(-31, otherHandle);
        commandHandler.put(-27, otherHandle);
        commandHandler.put(-8, otherHandle);
        Map.loadBoss(0);
        Map.loadBoss(1);
        Map.loadGhost();
    }

    private void stabilizeRegionRuntimeData() {
        Collection<Map> maps = mapList.values();

        for (Map map : maps) {
            if (map == null || map.nRegion <= 0 || map.ALL_REGION == null || map.ALL_REGION.isEmpty()) {
                continue;
            }

            for (int i = 0; i < map.ALL_REGION.size(); i++) {
                Region region = (Region) map.ALL_REGION.get(i);
                if (region == null) {
                    continue;
                }

                this.ensureRegionVectorField(region, "allPlayers", 3, true);
                this.ensureRegionVectorField(region, "allPlayerMessages", 3, true);
                this.ensureRegionVectorField(region, "EFFECT_AUTO", 3, true);
                this.ensureRegionVectorField(region, "tempMonster", 3, true);
                this.ensureRegionVectorField(region, "tempRemoveMonster", 3, true);
                this.ensureRegionVectorField(region, "monsters", 3, false);
                this.ensureRegionVectorField(region, "temp_monsters", 3, false);
            }
        }
    }

    private void ensureRegionVectorField(Region region, String fieldName, int expectedSize, boolean vectorElement) {
        try {
            Field field = Region.class.getDeclaredField(fieldName);
            field.setAccessible(true);
            Vector values = (Vector) field.get(region);

            if (values == null) {
                values = new Vector();
                field.set(region, values);
            }

            while (values.size() < expectedSize) {
                values.add(vectorElement ? new Vector() : new Hashtable());
            }
        } catch (Exception ex) {
        }
    }
    ...
    public void processMessage(Session session, Message message) {
        while (true) {
            try {
                if (maintenancePreparing || savingChar) {
                    try {
                        Thread.sleep(100L);
                ...
                if ((message.command == -31 || message.command == -32) && p != null && p.userID > 0 && p.isBot == -1 && p.map != null) {
                    p.map.notifyMap();
                ...
                Logger.logError(var8, "logs/runtime/errorlog.txt");
            }
    ...
    private void doMenuNpcNhanTieu(Char player, int idNpc, int idMenu, int idOptionMenu) {
        if (!Map.openvantieu) {
            ...
            try {
                ...
                if (!player.hasEscortTestAccess() && (ihour < 9 || ihour >= 21) && TeamServer.server != 100) {
    ...
    public void beginMaintenancePreparation() {
        maintenancePreparing = true;
        LoginHandler.stopLogin = true;
        this.wakeAllMaps();
    }

    private void wakeAllMaps() {
        Collection<Map> maps = mapList.values();
    ...
    private void settleMaintenanceQueue(long durationMs) {
    ...
    public void saveAllChar(boolean shutdownServer) {
        boolean previousSaving = savingChar;
        boolean previousPreparing = maintenancePreparing;
        boolean previousStopLogin = LoginHandler.stopLogin;
        System.out.println("savechar ");
        if (shutdownServer) {
            this.beginMaintenancePreparation();
            System.out.println("maintenance preparing: chan request moi, doi xu ly not queue");
            this.settleMaintenanceQueue(2000L);
        }
        ...
        try {
            ...
            if (!shutdownServer) {
                savingChar = previousSaving;
                maintenancePreparing = previousPreparing;
                LoginHandler.stopLogin = previousStopLogin;
                this.wakeAllMaps();
            }
        }
...
//                return;
        ...
        try {
            ...
            if (idMenu == 0 && idOptionMenu == 3) {
                if (UtilKPAH.getDayOfMonth() == Char.dayEndVip && UtilKPAH.getHour() >= Char.timeEndVip) {
                    player.sendMessage(MessageCreator.createServerAlertMessage("Không thể nhận thành tích trong thời gian này", ""));
                    return;
                }

                int[] diemxai = new int[]{0, 500};
                int[] diemnap = new int[]{500, 1000};
                i = Database.instance.countCharVip(2);
                int allCaCung = Database.instance.countCharVip(1);
                int[] var10000;
                ItemShop newItem;
                if (player.vip == 0) {
                    if (player.diemxaiVip >= diemxai[1] && player.diemNapVip >= diemnap[1] && i < MAX_VIP2) {
                        player.vip = 2;
                        var10000 = player.potions;
                        var10000[100] += Char.soluong_loa_vip[player.vip];
                        player.buy_luot_van_tieu = (byte) (player.buy_luot_van_tieu + 2);
                        newItem = new ItemShop((ItemTemplates) ((Hashtable) itemTemplates.get(5)).get(ID_THE_MUA_BAN), false, 0, 0, ID_THE_MUA_BAN);
                        newItem.id = player.getIDItem();
                        newItem.owner = player.charDBID;
                        ...
                        newItem.timeLoan = System.currentTimeMillis();
                        newItem.minuteExist = 43200;
                        newItem.dateCreate = Char.getDayTime(0L);
                        player.sendMessage(MessageCreator.createCharInventoryMessage(player, 0));
                        player.sendMessage(MessageCreator.createCharInventoryMessage(player, 1));
                        player.sendMessage(MessageCreator.createMsgChat(player.id, "Nhận được 2 loa và 1 thẻ mua bán thời hạn 30 ngày và thêm 2 lượt mua thẻ vận tiêu."));
                        Database.instance.saveOrtherLog("", player.charname, "nhan loa vip va the mua ban 30 ngay va them 2 luot mua the van tieu", "loavip");
                        if (ALL_CHAR_VIP.get(player.charname) == null) {
                            Database.instance.doAddCharVip(player.charname, player.vip);
                            ALL_CHAR_VIP.put(player.charname, Integer.valueOf(player.vip));
                        } else {
                            ALL_CHAR_VIP.put(player.charname, Integer.valueOf(player.vip));
                            Database.instance.doUpdateCharVip(player.charname, player.vip);
                        }
                    } else if (player.diemxaiVip >= diemxai[0] && player.diemNapVip >= diemnap[0] && allCaCung < MAX_VIP1) {
                        player.vip = 1;
                        ++player.buy_luot_van_tieu;
                        var10000 = player.potions;
                        var10000[100] += Char.soluong_loa_vip[player.vip];
                        player.sendMessage(MessageCreator.createCharInventoryMessage(player, 0));
                        player.sendMessage(MessageCreator.createMsgChat(player.id, "Nhận được 1 loa và thêm 1 lượt mua thẻ vận tiêu."));
                        Database.instance.saveOrtherLog("", player.charname, "nhan loa vip, them 1 luot mua the van tieu", "loavip");
                        if (ALL_CHAR_VIP.get(player.charname) == null) {
                            Database.instance.doAddCharVip(player.charname, player.vip);
                            ALL_CHAR_VIP.put(player.charname, Integer.valueOf(player.vip));
                        } else {
                        ...
                        player.sendMessage(MessageCreator.createMsgChat(player.id, VIP[1] + " và " + VIP[2] + " đã hết slot hoặc không đủ điều kiện"));
                    }
                } else if (player.vip == 1) {
                    if (player.diemxaiVip >= diemxai[1] && player.diemNapVip >= diemnap[1] && i < MAX_VIP2) {
                        player.vip = 2;
                        player.buy_luot_van_tieu = (byte) (player.buy_luot_van_tieu + 2);
                        var10000 = player.potions;
                        var10000[100] += Char.soluong_loa_vip[player.vip];
                        newItem = new ItemShop(itemTemplates.get(5).get(ID_THE_MUA_BAN), false, 0, 0, ID_THE_MUA_BAN);
                        newItem.id = player.getIDItem();
                        newItem.owner = player.charDBID;
                        newItem.level = newItem.getTemplate().level;
                        player.iItems.add(newItem);
                        ...
                        player.sendMessage(MessageCreator.createCharInventoryMessage(player, 1));
                        player.sendMessage(MessageCreator.createMsgChat(player.id, "Nhận được 2 loa và 1 thẻ mua bán thời hạn 30 ngày và thêm 2 lượt mua thẻ vận tiêu."));
                        Database.instance.saveOrtherLog("", player.charname, "nhan loa vip va the mua ban 30 ngay, them 2 luot mua the van tieu", "loavip");
                        if (ALL_CHAR_VIP.get(player.charname) == null) {
                            Database.instance.doAddCharVip(player.charname, player.vip);
                            ALL_CHAR_VIP.put(player.charname, Integer.valueOf(player.vip));
                        } else {
                        ...
                        player.sendMessage(MessageCreator.createMsgChat(player.id, VIP[2] + " đã hết slot hoặc không đủ điều kiện"));
                    }
                }
