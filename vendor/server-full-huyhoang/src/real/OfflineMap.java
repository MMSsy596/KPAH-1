...
public class OfflineMap extends Map {
   ...
   private boolean isPlayerTradeBlockedGem(final Actor item, final int type) {
      return type == 1 && item instanceof GemItem && ((GemItem)item).idGemTemplate == GemTemplate.AN_HAC_THI;
   }
         ...
         try {
            ...
            if (p != null) {
               ...
               if ((item = npcSell[player.inCountry][idNpc - 11].getOneItemOfUser(shopId, p, itemID, type)) != null) {
                  ...
                  if (this.isPlayerTradeBlockedGem(item, type)) {
                     npcSell[player.inCountry][idNpc - 11].removeItem(shopId, p, item, type, ((GemItem)item).realId);
                     item.isSelling = false;
                     player.sendMessage(MessageCreator.createServerAlertMessage("An Hac Thi la vat pham khoa, khong the trao doi.", ""));
                     return;
                  }
         ...
         if (!this.checkTrade(player)) {
            try {
               ...
               if (type == 0) {
                  ...
                  if (item == null) {
                     player.sendMessage(MessageCreator.createServerAlertMessage("Khong tim thay vat pham.", ""));
                     return;
                  }
