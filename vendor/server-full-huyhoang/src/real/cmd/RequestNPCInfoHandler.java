...
public class RequestNPCInfoHandler implements ICommandHandler {
    ...
    private static String[] getNongDanNpcMenu(final Char p) {
        if (LuongSon108Manager.instance.isWorldRaidActive()) {
            return new String[]{
                    "Ngan hang doi diem",
                    "Thong bao",
                    "Quy tai chinh",
                    "Gian hang nong dan (" + p.diemNongDan + " diem)",
                    "Truy bat 108 Luong Son"
            };
        }
        return new String[]{"Ngan hang doi diem", "Thong bao", "Quy tai chinh", "Gian hang nong dan (" + p.diemNongDan + " diem)"};
    }

    private static String[] getBlackMarketVendorMenu(final Char p, final int npcType) {
        if (p != null && p.map != null) {
            return p.map.getBlackMarketVendorMenuPreview(p, npcType);
        }
        return new String[]{"Chợ Đen"};
    }
    ...
    public void process(final Session session, final Message message) throws IOException {
        ...
        Message msg = null;
        String[] menu = {""};
        if (Map.isBlackMarketVendorNpcId(npcType)) {
            p = CharManager.instance.getByUserID(session.userID);
            if (p != null) {
                menu = getBlackMarketVendorMenu(p, npcType);
                msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                session.sendMessage(msg);
            }
            return;
        }
        if (npcType == Map.ID_NPC_GIAN_HANG_NONG_DAN) {
            p = CharManager.instance.getByUserID(session.userID);
            if (p != null) {
                menu = getNongDanNpcMenu(p);
                msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                session.sendMessage(msg);
            }
            return;
        ...
        if (npcType == Map.ID_NPC_GIAN_HANG_NONG_DAN) {
            p = CharManager.instance.getByUserID(session.userID);
            if (p != null) {
                menu = new String[]{"Ngân hàng", "Thông báo", "Phật tổ 15p (20k xu)", "Bàn cổ 15p (40k xu)", "Gian hàng nông dân (" + p.diemNongDan + " điểm)"};
                msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                session.sendMessage(msg);
            }
            return;
        }
        if (npcType == Map.ID_NPC_TU_QUAN_THU_PHI) {
            p = CharManager.instance.getByUserID(session.userID);
            if (p != null) {
                menu = LuongSon108Manager.instance.buildTollMenu(p);
                msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                session.sendMessage(msg);
            }
            return;
        ...
        if (npcType == -10) {
            p = CharManager.instance.getByUserID(session.userID);
            if (p != null) {
                menu = new String[]{"Hop luc", "Mo rong hanh trang(150L)", "Thu van may", "Chuyen lanh tho(300L)", "Nhan qua Giftcode", "Dang ky lien dau"};
                if (p.canReceiveGiftOffline > -1) {
                    menu = new String[]{"Hop luc", "Mo rong hanh trang(150L)", "Thu van may", "Chuyen lanh tho(300L)", "Nhan qua Giftcode", "Dang ky lien dau", "Nhan qua top"};
                }
                if (p.canReceiveGiftEvent()) {
                    menu = new String[]{"Hop luc", "Mo rong hanh trang(150L)", "Thu van may", "Chuyen lanh tho(300L)", "Nhan qua Giftcode", "Dang ky lien dau", "Nhan qua"};
                }
                if (p.maxBag > 1) {
                    menu = new String[]{"Hop luc", "Mo rong hanh trang(500L)", "Thu van may", "Chuyen lanh tho(300L)", "Nhan qua Giftcode", "Dang ky lien dau"};
                    if (p.canReceiveGiftOffline > -1) {
                        menu = new String[]{"Hop luc", "Mo rong hanh trang(500L)", "Thu van may", "Chuyen lanh tho(300L)", "Nhan qua Giftcode", "Dang ky lien dau", "Nhan qua top"};
                    }
                    if (p.canReceiveGiftEvent()) {
                        menu = new String[]{"Hop luc", "Mo rong hanh trang(500L)", "Thu van may", "Chuyen lanh tho(300L)", "Nhan qua Giftcode", "Dang ky lien dau", "Nhan qua"};
                    }
                }
                if (!TeamServer.isServerLienDau()) {
                    final boolean isReg = Database.instance.isCharRegisterLiendau(p.charname);
                    if (isReg) {
                        menu[5] = "Cap nhat thong tin";
                    }
                }
                if (TeamServer.isServerLienDau()) {
                    menu = new String[]{"Hop luc", "Mo rong hanh trang(500L)", "Thu van may", "Chuyen lanh tho(300L)", "Nhan qua Giftcode", "Vao chien truong", "Ket qua"};
                }
                msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                session.sendMessage(msg);
            }
        ...
        if (npcType == -11) {
            p = CharManager.instance.getByUserID(session.userID);
            msg = MessageCreator.createMsgMenu(new String[]{"Nguyen lieu thuong", "Nguyen lieu khoa", "Ngoc huyen minh", "Ngoc huyen minh khoa", "Bot thuong", "Bot khoa"}, npcType, 0);
            if (p != null && p.hlt_buy > 0) {
                msg = MessageCreator.createMsgMenu(new String[]{"Nguyen lieu thuong", "Nguyen lieu khoa", "Ngoc huyen minh", "Ngoc huyen minh khoa", "Bot thuong", "Bot khoa", "Huyet linh thao(" + p.hlt_buy + ")"}, npcType, 0);
            }
            session.sendMessage(msg);
            return;
        }
        if (npcType == -12) {
            msg = MessageCreator.createMsgMenu(new String[]{"Nguyen lieu thuong", "Nguyen lieu khoa", "Xuong khong khoa", "Xuong khoa"}, npcType, 0);
            session.sendMessage(msg);
            return;
        }
        if (npcType == Map.ID_NPC_GIAN_HANG_NONG_DAN) {
        ...
        switch (npcType) {
            ...
            case -2: {
                p = CharManager.instance.getByUserID(session.userID);
                if (p != null) {
                    menu = new String[]{"Hỗ Trợ","Đang Cập Nhật"};
            ...
            case Map.ID_NPC_GIAN_HANG_NONG_DAN: {
                p = CharManager.instance.getByUserID(session.userID);
                if (p != null) {
                    menu = new String[]{"Gian hàng nông dân (" + p.diemNongDan + " điểm)"};
                    msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                    session.sendMessage(msg);
                    return;
                }
...
//                    session.sendMessage(msg);
//                }
                if (p != null && (p.luot_van_tieu > 0 || p.hasEscortTestAccess())) {
                    if (p.myCountry == p.inCountry) {
                        ...
                        if (p.hasEscortTestAccess()) {
                            menu[0] = "Nhận tiêu cá nhân(test)";
                        }
                        msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                        session.sendMessage(msg);
                    }
            ...
            case -49: { // todo npc sự kiện
                ...
                if (p != null) {
                    ...
                    if (!Char.isFinishAllSuKienGioTo2016()) {
                        menu = new String[]{"Đổi quà", "Đổi nguyên liệu", "Đổi điểm ND"};
                        if (TeamServer.isServerHoaLu() || TeamServer.isServerLocal()) {
                            menu = new String[]{"Đổi quà", "Đổi nguyên liệu", "Đổi điểm ND", "Điểm dùng lượng(" + p.luongxainhanRuong + ")", "Đổi rương bát bảo(" + p.luongxai + " điểm)", "Đổi xu"};
                        }
                    }
            ...
            case -34: {
                ...
                if (p != null && p.map.nRegion > 0) {
                    ...
                    for (int i = 0; i < p.map.nRegion; ++i) {
                        menu[i] = Map.getRegionDisplayName(p.mapID, i, p.map.nRegion);
                    }
                    msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                    session.sendMessage(msg);
                }
            ...
            case -25: {
                ...
                if (p != null && p.myCountry > -1 && p.idClan > -1 && Map.idClanTown[p.myCountry] == p.idClan) {
                    ...
                    if (cl != null && cl.master.equals(p.charname)) {
                        final String hourShieldCostLabel = Map.getClanTerritoryShieldSlotCostLabel();
                        final String allShieldCostLabel = Map.getClanTerritoryShieldAllCostLabel();
                        final String[] hour = {"Bảo hộ 6h-10h (" + hourShieldCostLabel + ")", "Bảo hộ 11h-15h (" + hourShieldCostLabel + ")", "Bảo hộ 16h-20h (" + hourShieldCostLabel + ")", "Bảo hộ 20h-24h (" + hourShieldCostLabel + ")"};
                        menu = new String[]{hour[Char.getIdOpenShield(Calendar.getInstance().get(11))], "Bảo hộ toàn bộ (" + allShieldCostLabel + ")", "Đăng ký tấn công lãnh thổ"};
            ...
            case -8: {
                ...
                if (p != null) {
                    ...
                    menu = new String[]{"Vũ khí", "Trang bị", "Trang bị thú",
                        "Nhận thành tích(ĐN " + p.diemNapVip + " , ĐT " + p.diemxaiVip + ", MN: " + (p.luongNap * 2) + ")",
                        "Hoạt động giờ",
                    ...
                    String[] menuExtend = new String[menu.length + 1];
                    System.arraycopy(menu, 0, menuExtend, 0, menu.length);
                    menuExtend[menu.length] = "Sua do nhanh";
                    menu = menuExtend;
                    msg = MessageCreator.createMsgMenu(menu, npcType, 0);
                    session.sendMessage(msg);
                }
