        ...
        if (!player.isCorrectPass && !player.passWord.equals("")) {
            ...
            if (!player.map.checkTrade(player)) {
                try {
                    ...
                    if (item.id == Map.ID_SHOP_EXP_LOCK_CARD) {
                        if (item.sell == 0) {
                            player.sendMessage(MessageCreator.createServerAlertMessage("Vat pham hien tai chua ban.", ""));
                            return;
                        }

                        if (player.potions[Map.ID_POTION_EXP_LOCK_CARD] + item.value > 999) {
                            player.sendMessage(MessageCreator.createServerAlertMessage("Khong the mua them.", ""));
                            return;
                        }

                        if (player.getLuongLock() < item.getPrice(isSale)) {
                            player.sendMessage(MessageCreator.createServerAlertMessage("Khong du luong khoa.", ""));
                            return;
                        }

                        player.subLuongLock((long) item.getPrice(isSale));
                        player.potions[Map.ID_POTION_EXP_LOCK_CARD] += item.value;
                        ...
                        player.sendMessage(MessageCreator.createServerAlertMessage("Da mua. Vat pham dang o trong hanh trang.", ""));
                        Message m = new Message(85);
                        player.sendMessage(m);
                        m.cleanup();
                        Database.instance.saveLogBuyItemShop(player.charname, item.name, item.type, item.getPrice(isSale), "Luong_khoa_" + player.getLuongLock());
                        Database.instance.saveLogThongKeChiTieu(item.name, 0, item.value, item.getPrice(isSale), "luong_lock");
                        return;
                    }
                    if (item.id == Map.ID_SHOP_DURABILITY_BLESSING) {
                        if (item.sell == 0) {
                            player.sendMessage(MessageCreator.createServerAlertMessage("Váº­t pháº©m hiá»‡n táº¡i chÆ°a bÃ¡n.", ""));
                            return;
                        }

                        if (player.potions[Map.ID_POTION_DURABILITY_BLESSING] + item.value > 999) {
                            player.sendMessage(MessageCreator.createServerAlertMessage("Khong the mua them.", ""));
                            return;
                        }

                        if (player.getxu() < (long) item.getPrice(isSale)) {
                            player.sendMessage(MessageCreator.createServerAlertMessage("KhÃ´ng Ä‘á»§ xu.", ""));
                            return;
                        }

                        player.subXu((long) item.getPrice(isSale), true, "mua bua giam hao mon");
                        player.potions[Map.ID_POTION_DURABILITY_BLESSING] += item.value;
                        player.sendMessage(MessageCreator.createMainCharInfoMessage(player));
                        player.sendMessage(MessageCreator.createCharInventoryMessage(player, 0));
                        player.sendMessage(MessageCreator.createServerAlertMessage("Da mua. Vat pham dang o trong hanh trang.", ""));
                        Message m = new Message(85);
                        player.sendMessage(m);
                        m.cleanup();
                        Database.instance.saveLogBuyItemShop(player.charname, item.name, item.type, item.getPrice(isSale), "Xu_" + player.getxu());
                        Database.instance.saveLogThongKeChiTieu(item.name, 0, item.value, item.getPrice(isSale), "xu");
                        return;
                    }
                    ...
                    if (player.map.isItemLock(item.id)) {
                        ...
                        if ((item.id < 116 || item.id > 125) && item.id != 150 && item.id != 176) {
                            ...
                            if (item.id == 176) {
                                ...
                                player.doAddGemItem(idgem[item.id - 116], item.value, true);
                                nameItem = item.name;
                            }
