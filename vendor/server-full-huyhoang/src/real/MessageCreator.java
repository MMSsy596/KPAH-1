...
public class MessageCreator {
    ...
    private static int countUtf8RepairMarkers(final String text) {
        if (text == null) {
            return 0;
        }
        int count = 0;
    ...
    private static String repairUtf8(final String text, final java.nio.charset.Charset sourceCharset) {
        return new String(text.getBytes(sourceCharset), java.nio.charset.StandardCharsets.UTF_8);
    }

    private static String chooseBestUtf8RepairCandidate(final String text) {
        final int currentScore = countUtf8RepairMarkers(text);
        String best = text;
        int bestScore = currentScore;
        try {
            final String repairedCp1252 = repairUtf8(text, java.nio.charset.Charset.forName("windows-1252"));
            final int repairedCp1252Score = countUtf8RepairMarkers(repairedCp1252);
            if (repairedCp1252Score < bestScore) {
                best = repairedCp1252;
                bestScore = repairedCp1252Score;
            }
        } catch (Exception ignored) {
        }
        try {
            final String repairedLatin1 = repairUtf8(text, java.nio.charset.StandardCharsets.ISO_8859_1);
            final int repairedLatin1Score = countUtf8RepairMarkers(repairedLatin1);
            if (repairedLatin1Score < bestScore) {
                best = repairedLatin1;
            }
        } catch (Exception ignored) {
        }
    ...
    private static String repairUtf8Chunk(final String text) {
        String fixed = text;
        for (int i = 0; i < 5 && needsUtf8Repair(fixed); ++i) {
            final String best = chooseBestUtf8RepairCandidate(fixed);
            if (best.equals(fixed)) {
                break;
            }
            fixed = best;
        }
    ...
    private static boolean isUtf8ChunkBoundary(final char ch) {
        return "\"'`()[]{}<>|/\\\\,;:!?+-=*.".indexOf(ch) >= 0;
    }

    private static String repairMixedUtf8Text(final String text) {
        StringBuilder sb = new StringBuilder(text.length());
        int chunkStart = -1;
        for (int i = 0; i < text.length(); ++i) {
            final char ch = text.charAt(i);
            if (Character.isWhitespace(ch) || isUtf8ChunkBoundary(ch)) {
                if (chunkStart >= 0) {
                    sb.append(repairUtf8Chunk(text.substring(chunkStart, i)));
                    chunkStart = -1;
                }
                sb.append(ch);
                continue;
        ...
        if (chunkStart >= 0) {
            sb.append(repairUtf8Chunk(text.substring(chunkStart)));
        }
        return sb.toString();
    }

    private static boolean needsUtf8Repair(final String text) {
        if (countUtf8RepairMarkers(text) > 0) {
            return true;
        }
        return text != null
                && (text.indexOf('Ã') >= 0
    ...
    private static String normalizeDisplayText(final String text) {
        if (text == null) {
            return "";
        }
        final String fixed = repairMixedUtf8Text(text);
        return fixed.replace('\r', ' ');
    }

    public static String normalizeUiText(final String text) {
        return Normalizer.normalize(normalizeDisplayText(text), Normalizer.Form.NFC);
    }

    private static String toClientSafeText(final String text) {
        String fixed = normalizeDisplayText(text);
        fixed = fixed.replace('Đ', 'D').replace('đ', 'd');
        fixed = Normalizer.normalize(fixed, Normalizer.Form.NFD).replaceAll("\\p{M}+", "");
        fixed = fixed.replaceAll("[^\\p{ASCII}]", " ");
        fixed = fixed.replaceAll("\\s+", " ").trim();
        return fixed;
    }

    private static String toClientAsciiText(final String text) {
        String fixed = normalizeDisplayText(text);
        fixed = fixed.replace('\u0110', 'D').replace('\u0111', 'd');
        fixed = Normalizer.normalize(fixed, Normalizer.Form.NFD).replaceAll("\\p{M}+", "");
        fixed = fixed.replaceAll("[^\\p{ASCII}]", " ");
    ...
    // UI fonts on the client support only this Vietnamese subset reliably.
    private static final String CLIENT_UI_GLYPHS =
            " 0123456789.,:!?()+*$#/-%abcdefghijklmnopqrstuvwxyz"
                    ...
                    + "úùủũụưứừửữự"
                    + "ýỳỷỹỵđ"
                    + "ABCDEFGHIJKLMNOPQRSTUVWXYZĐ~ĂÁ";

    // Overhead names on map use a much smaller font, so keep them ASCII lowercase-safe.
    private static final String CLIENT_OVERHEAD_GLYPHS = "0123456789+-./abcdefghijklmnopqrstuvwxyz:@ ";

    private static String sanitizeForClientGlyphs(final String text, final String allowedGlyphs, final boolean forceLowerAscii) {
        String fixed = normalizeDisplayText(text);
        fixed = Normalizer.normalize(fixed, Normalizer.Form.NFC);
        StringBuilder sb = new StringBuilder(fixed.length());
        for (int i = 0; i < fixed.length(); i++) {
            ...
            if (forceLowerAscii && ch >= 'A' && ch <= 'Z') {
                ch = (char) (ch + 32);
            }
            if (allowedGlyphs.indexOf(ch) >= 0) {
                sb.append(ch);
                continue;
            }
            char fallback = toFallbackClientGlyph(ch, forceLowerAscii);
            sb.append((allowedGlyphs.indexOf(fallback) >= 0) ? fallback : ' ');
        }
    ...
    private static char toFallbackClientGlyph(final char ch, final boolean forceLowerAscii) {
        if (ch == '\u0110' || ch == '\u0111') {
            return forceLowerAscii ? 'd' : ((ch == '\u0110') ? 'D' : 'd');
        }
        String decomposed = Normalizer.normalize(String.valueOf(ch), Normalizer.Form.NFD).replaceAll("\\p{M}+", "");
        for (int i = 0; i < decomposed.length(); i++) {
            char candidate = decomposed.charAt(i);
            if (forceLowerAscii && candidate >= 'A' && candidate <= 'Z') {
                candidate = (char) (candidate + 32);
            }
            if ((candidate >= 'a' && candidate <= 'z')
                    || (!forceLowerAscii && candidate >= 'A' && candidate <= 'Z')
    ...
    private static String toClientDisplayText(final String text) {
        return sanitizeForClientGlyphs(text, CLIENT_UI_GLYPHS, false);
    }

    private static String toClientOverheadText(final String text) {
        return sanitizeForClientGlyphs(text, CLIENT_OVERHEAD_GLYPHS, true);
    }
    ...
    public static Message createMsgChat(final int id, final String msg) {
        ...
        try {
            m.dos.writeShort(id);
            m.dos.writeUTF(toClientDisplayText(msg));
    ...
    public static Message createMsgNpc(final String name, final int x, final int y, final int w, final int h, final int nFrame, final int idImg, final int id, final byte type) {
        ...
        try {
            m.dos.writeUTF(toClientOverheadText(name));
            m.dos.writeShort(id);
    ...
    public static Message createMsgSound(final int type) {
        ...
        try {
            ...
            if (type == 1) {
                ...
                m.dos.writeUTF(toClientDisplayText("Bạn có muốn bật nhạc không"));
                m.dos.writeByte(0);
            }
        } catch (final Exception e) {
            e.printStackTrace();
    ...
    public static Message createMsgMenu(final Vector<String> menu, final int idNPC, final int idmenu) {
        ...
        try {
            ...
            for (int i = 0; i < menu.size(); ++i) {
                msg.dos.writeUTF(toClientDisplayText(menu.get(i)));
            }
        } catch (final Exception ex) {
        }
        return msg;
    ...
    public static Message createMsgMenu(final String[] menu, final int idNPC, final int idmenu) {
        ...
        try {
            ...
            for (int i = 0; i < menu.length; ++i) {
                msg.dos.writeUTF(toClientDisplayText(menu[i]));
            }
        } catch (final Exception ex) {
        }
        return msg;
    ...
    public static Message createMsgInputText(final int idNPC, final int idmenu, final String info, final int typeIput) {
        ...
        try {
            msg.dos.writeShort(idNPC);
            msg.dos.writeByte(idmenu);
            msg.dos.writeUTF(toClientDisplayText(info));
        ...
        try {
            ...
            for (int i = 0; i < chars.size(); ++i) {
                ...
                if (charData.timeDell != 0L && charData.using == 0) {
                    day = Math.max(0, 3 - (int) ((time - charData.timeDell) / 1000L / 60L / 60L / 24L));
                }
            ...
            if (type == 2) {
                ...
                for (int l = 0; l < p.animal.size(); ++l) {
                    ...
                    if (animal.place != 1) {
                        ...
                        m.dos.writeUTF(p.getAnimalInfoForClient(animal));
                        m.dos.writeByte(0);
                    }
                }
    ...
    public static Message createCharWearingMessage(final Char p, final Char charGet) {
        final Message m = new Message(15);
        try {
            final Session viewSession = (charGet != null && charGet.getSession() != null) ? charGet.getSession() : p.getSession();
            final boolean version180 = viewSession != null && viewSession.isVersion180();
            final boolean useShortPetId = viewSession != null && viewSession.version.compareTo("2.2.5") >= 0;
            m.dos.writeByte(0);
            m.dos.writeShort(p.id);
            Vector<Item> charwearing = new Vector<Item>();
            if (p.getCharWearing().size() == 0) {
            ...
            if (p.petUsing != null) {
                type = p.petUsing.idImg;
                if (p.petUsing.getType() == Pet.TYPE_PET_TOOL && version180) {
                    type = p.petUsing.getIdEffPet();
                }
            }
            m.dos.writeByte(type);
    ...
    public static void createMsgCharMonsterCopy(final Char ch, final Char request) {
     ...
     public static Message createCharInfo(final Char ch) {
        ...
        try {
            ...
            m.dos.writeUTF(toClientDisplayText(name));
            ...
            m.dos.writeShort(ch.getDefendPhysic() + ch.percentBuff[0]);
            m.dos.writeShort(ch.getDefendMagic() + ch.getBuffDefCB(1, true));
            ...
            m.dos.writeByte(AmbientBotManager.getDisplayBotFlag(ch));
            if (ch.idClan > -1) {
    ...
    public static Message createMainCharInfoMessage(final Char p) throws IOException {
        ...
        m.dos.writeShort(p.getLuckyValue() + p.getEffSkillClan(3) + p.upAbility[4]);
    ...
    public static Message createServerInfoMessage() throws IOException {
        final Message m = new Message(38);
        m.dos.writeUTF(toClientDisplayText(RealController.maquee));
        return m;
    }
    
    public static Message createthongbao(final String message) throws IOException {
        final Message m = new Message(38);
        m.dos.writeUTF(toClientDisplayText(message));
        return m;
    }

    public static Message createServerInfoMessage1() throws IOException {
        final Message m = new Message(38);
        m.dos.writeUTF(toClientDisplayText(RealController.maquee2));
        return m;
    }
    ...
    public static Message createServerAlertMessage(final String message, final String url) {
        ...
        try {
            m.dos.writeUTF(toClientDisplayText(message));
            m.dos.writeUTF(toClientDisplayText(url));
        } catch (final Exception ex) {
        }
        return m;
    }
            ...
            for (int i = 0; i < allmem.size(); ++i) {
                ...
                if (charinfo == null) {
                    ...
                    if (Map.applyClanLowMemberAutoDisbandProtection(clanInfo)) {
                        Database.instance.updateTimeCheckDelClan(clanInfo.id, clanInfo.timeStartDel);
                    }
                    Database.instance.updateCharNewClan(clanInfo);
                } else {
