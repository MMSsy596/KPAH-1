            ...
            public void run() {
                while (!AdminHandler.isStopServer) {
                    try {
                        ...
                        playerMessages = (Vector)MaterialMap.this.allPlayerMessages.get(1);
                        while (playerMessages.size() > 0) {
                            PlayerMessage pm = (PlayerMessage)playerMessages.remove(0);
                            if (!pm.player.exit) {
                                MaterialMap.this.processMessage(pm.player, pm.message);
                            }
                    ...
                    try {
                        Object object = MaterialMap.this.LOCK;
                        synchronized (object) {
                            MaterialMap.this.LOCK.wait(timeDelay);
                        }
                    }
                    catch (Exception exception) {
                        // empty catch block
            ...
            public void run() {
                while (!AdminHandler.isStopServer) {
                    try {
                        int count = 0;
                        Vector playerMessages = new Vector();
                        playerMessages = (Vector)MaterialMap.this.allPlayerMessages.get(2);
                        while (playerMessages.size() > 0) {
                            PlayerMessage pm = (PlayerMessage)playerMessages.remove(0);
                            if (!pm.player.exit) {
                                MaterialMap.this.processMessage(pm.player, pm.message);
                    ...
                    try {
                        Object object = MaterialMap.this.LOCK1;
                        synchronized (object) {
                            MaterialMap.this.LOCK1.wait(timeDelay);
                        }
                    }
                    catch (Exception exception) {
                        // empty catch block
            ...
            public void run() {
                while (!AdminHandler.isStopServer) {
                    try {
                        int count = 0;
                        Vector playerMessages = new Vector();
                        playerMessages = (Vector)MaterialMap.this.allPlayerMessages.get(0);
                        while (playerMessages.size() > 0) {
                            PlayerMessage pm = (PlayerMessage)playerMessages.remove(0);
                            if (!pm.player.exit) {
                                MaterialMap.this.processMessage(pm.player, pm.message);
                    ...
                    try {
                        Object object = MaterialMap.this.LOCK2;
                        synchronized (object) {
                            MaterialMap.this.LOCK2.wait(timeDelay);
                        }
                    }
                    catch (Exception exception) {
                        // empty catch block
