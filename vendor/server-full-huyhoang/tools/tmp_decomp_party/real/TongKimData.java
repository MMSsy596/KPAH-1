...
extends Thread {
    ...
    public static final byte TYPE_BATTLE = 5;
    public static final byte TYPE_BATTLE_MULTI = 6;
    ...
    public TongKimData(int typeParty, MapTongKim map) {
        this.parrent = map;
    ...
    public void playerExit(Char p) {
        ...
        while (i < this.players.size()) {
            if (this.players.get((int)i).charname.toLowerCase().equals(p.charname.toLowerCase())) {
                ...
                if (getXp > 0) {
                    ...
                    if ((totalXp = dxp) > 0) {
                        ...
                        if (newxp != totalXp) {
                            ...
                            while (i < p.party.userParty.size()) {
                                ...
                                if (pp.id != p.id && p.near((Actor)pp, 320) && pp.mapID == p.mapID && pp.inCountry == p.inCountry) {
                                    ...
                                    if (pp.hp > 0) {
                                        ...
                                        Map.addXPForChar((Char)pp, (long)(temp + pp.getEffSkillClan(0) * temp / 100), (boolean)false, (String)"TongKimData1");
                                    }
                                }
                                ++i;
                            }
                            ...
                            Map.addXPForChar((Char)p, (long)(xpReceive + p.getEffSkillClan(0) * xpReceive / 100), (boolean)false, (String)"TongKimData2");
                        } else {
                            totalXp *= doubleALL;
                            totalXp = p.expReceive(totalXp);
                            Map.addXPForChar((Char)p, (long)(totalXp + p.getEffSkillClan(0) * totalXp / 100), (boolean)false, (String)"TongKimData3");
                        }
                    }
                }
                m = new Message(9);
                ...
                if (newxp != totalXp) {
                    ...
                    while (k < p.party.userParty.size()) {
                        ...
                        if (pp.id != p.id && p.near((Actor)pp, 320) && pp.mapID == p.mapID && pp.inCountry == p.inCountry) {
                            ...
                            if (pp.hp > 0) {
                                temp = pp.expReceive(temp);
                                Map.addXPForChar((Char)pp, (long)(temp + pp.getEffSkillClan(0) * temp / 100), (boolean)false, (String)"TongKimData4");
                            }
                        }
                        ++k;
                    }
                    ...
                    xpReceive = p.expReceive(xpReceive);
                    Map.addXPForChar((Char)p, (long)(xpReceive + p.getEffSkillClan(0) * xpReceive / 100), (boolean)false, (String)"TongKimData5");
                } else {
                    totalXp = p.expReceive(totalXp);
                    Map.addXPForChar((Char)p, (long)(totalXp + p.getEffSkillClan(0) * totalXp / 100), (boolean)false, (String)"TongKimData6");
                }
            }
...
lbl7:
                    ...
                    if (this.timeOut()) {
                        if (this.sendAlert == 0) {
                            ...
                            if (teamwin == 2 || teamwin == -1) {
                                ...
                                if (this.typeArena != 3) {
                                    this.sendAllCharMap(MessageCreator.createServerAlertMessage((String)("Ch\u00fac m\u1eebng \u0111\u1ed9i " + TongKimData.nameTeam[teamwin] + " \u0111\u00e3 chi\u1ebfn th\u1eafng."), (String)""));
                                } else {
                            ...
                            this.resetAll();
                        } else if (this.sendAlert == 1 && System.currentTimeMillis() - this.timeStart > (long)(TongKimData.timeFight + 60000)) {
                            this.doSendResult(0);
                            this.sendAlert = 2;
                        }
                        if (System.currentTimeMillis() - this.timeStart > (long)((TongKimData.timeFight + 1) * 60000)) {
    ...
    public void onDraw() {
        ...
        if (this.typeArena == 3) {
            ...
            for (String name : t.memTeam.values()) {
                ...
                try {
                    c1.addXu((long)(this.moneyThachdau / 2), "tongkimdata 1");
            ...
            for (String name : t.memTeam.values()) {
                ...
                try {
                    c2.addXu((long)(this.moneyThachdau / 2), "tongkimdata 2");
            ...
            try {
                Database.instance.saveOrtherLog("", c1.charname, info, "darena");
            }
            catch (Exception exception) {
                // empty catch block
            }
            try {
                Database.instance.saveOrtherLog("", c2.charname, info, "darena");
            }
            catch (Exception exception) {
                // empty catch block
            }
    ...
    public void doGetPotion(Char player, short id) throws IOException {
        ...
        if (pt != null) {
            if ((pt.getType() == 0 || Map.isPotionUnlimit((short)pt.getType()) || player.potions[pt.getType()] < 999 && pt.getType() != 85 && pt.getType() != 80 && pt.getType() != 88 || pt.getType() == 78 || pt.getType() == 80 || pt.getType() == 35) && (pt.belongUser == player.charDBID || pt.belongUser == 0)) {
                ...
                if (pt.getType() == 0) {
                    player.addXu((long)(pt.quantity + pt.quantity * player.getEffSkillClanMember(0) / 100), "tongkimdata 3");
                }
    ...
    private void doGetPotion(Char player, Message message) throws IOException {
        ...
        if (pt != null) {
            if ((pt.getType() == 0 || Map.isPotionUnlimit((short)pt.getType()) || player.potions[pt.getType()] < 999 && pt.getType() != 78 && pt.getType() != 85 && pt.getType() != 80 && pt.getType() != 35 || pt.getType() == 78 || pt.getType() == 80 || pt.getType() == 35) && (pt.belongUser == player.charDBID || pt.belongUser == 0)) {
                ...
                player.potions[s] = player.potions[s] + pt.quantity;
                if (pt.getType() == 0) {
                    player.addXu((long)(pt.quantity + pt.quantity * player.getEffSkillClanMember(0) / 100), "tongkimdata 4");
                }
    ...
    public void setTeam(Char player) {
        ...
        for (String name : t.memTeam.values()) {
            p = CharManager.instance.getCharByCharName(name);
            if (p == null || !p.charname.toLowerCase().equals(player.charname.toLowerCase())) continue;
            p = player;
            player.pk = (byte)14;
            try {
            ...
            block15: {
                block14: {
                    ...
                    block11: {
                        block13: {
                            block12: {
                                p1 = this.team.get((int)0).totalPoint;
                                p2 = this.team.get((int)1).totalPoint;
                                if (this.typeArena != 0) break block10;
                                if (!this.timeOut()) break block11;
                                if (p1 <= p2) break block12;
                                this.infoResult = "\u0110\u1ed8I \u0110\u1ece TH\u1eaeNG";
                                return 0;
                            }
                            if (p1 >= p2) break block13;
                    ...
                    d1 = p1 / 10;
                    d2 = p2 / 10;
                    if (d1 < MAX_WIN || d2 >= MAX_WIN) break block14;
                    this.timeStart = System.currentTimeMillis() - (long)(timeFight * 60000);
                    this.infoResult = "\u0110\u1ed8I \u0110\u1ece TH\u1eaeNG";
                    return 0;
                }
                if (d1 >= MAX_WIN || d2 < MAX_WIN) break block15;
                this.timeStart = System.currentTimeMillis() - (long)(timeFight * 60000);
                this.infoResult = "\u0110\u1ed8I XANH TH\u1eaeNG";
                return 1;
            }
    ...
    public byte checkWin(Char plose, Char pWin) {
        if (plose != null) {
            ...
            this.infoResult = String.valueOf(pWin.charname) + " chi\u1ebfn th\u1eafng";
            this.infoWin = String.valueOf(pWin.charname) + " \u0111\u00e3 chi\u1ebfn th\u1eafng " + plose.charname;
            Database.instance.saveOrtherLog("", pWin.charname, this.infoWin, "winarena");
            Database.instance.saveOrtherLog("", plose.charname, this.infoWin, "losearena");
        }
        return this.win;
    }
        ...
        for (String name : t.memTeam.values()) {
            ...
            try {
                ...
                if (p != null) {
                    ...
                    info = String.valueOf(p.charname) + " chien thang " + pLose + " va nhan dc " + this.moneyThachdau + " xu";
                    p.addXu((long)this.moneyThachdau, "tongkimdata 5");
    ...
    public void checkKhan(Char p) {
        ...
        try {
            t2 = this.team.get(0);
            if (t2.memTeam.get(p.charname) != null && p.pk != 14) {
                p.pk = (byte)14;
                Message m = new Message(65);
                m.dos.writeShort(p.id);
                m.dos.writeByte(1);
        ...
        try {
            t2 = this.team.get(1);
            if (t2.memTeam.get(p.charname) != null && p.pk != 15) {
                p.pk = (byte)15;
                Message m = new Message(65);
                m.dos.writeShort(p.id);
                m.dos.writeByte(1);
    ...
    public void startMatch() {
        if (this.sendInfoMatch) {
            ...
            for (String name : t.memTeam.values()) {
                ...
                try {
                    if (this.typeArena != 3) {
                        c1.map.move2Map(c1, 24 + Map.r.nextInt() % 3, 10 + Map.r.nextInt() % 3, 207, (int)c1.myCountry);
                        continue;
                    }
                    c1.map.move2Map(c1, 22, 9, 207, (int)c1.myCountry);
                }
                catch (Exception exception) {
                    // empty catch block
                }
            ...
            for (String name : t.memTeam.values()) {
                ...
                try {
                    if (this.typeArena != 3) {
                        c2.map.move2Map(c2, 24 + Map.r.nextInt() % 3, 36 + Map.r.nextInt() % 3, 207, (int)c2.myCountry);
                        continue;
                    }
                    c2.map.move2Map(c2, 22, 37, 207, (int)c2.myCountry);
                }
                catch (Exception exception) {
                    // empty catch block
                }
    ...
    public void onActorDie(LiveActor ac) {
        if (this.typeArena == 3) {
            ...
            this.infoWin = String.valueOf(nameWin) + " \u0111\u00e3 chi\u1ebfn th\u1eafng " + plose.charname;
