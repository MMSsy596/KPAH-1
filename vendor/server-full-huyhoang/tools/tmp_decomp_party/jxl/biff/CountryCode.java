...
public class CountryCode {
    private static Logger logger = Logger.getLogger(class$jxl$biff$CountryCode == null ? (class$jxl$biff$CountryCode = CountryCode.class$("jxl.biff.CountryCode")) : class$jxl$biff$CountryCode);
    private int value;
    private String code;
    ...
    private CountryCode(int v, String c, String d) {
        ...
        newcodes[CountryCode.codes.length] = this;
        codes = newcodes;
    }
    ...
    public static CountryCode getCountryCode(String s) {
        ...
        for (int i = 0; i < codes.length && code == UNKNOWN; ++i) {
            if (!CountryCode.codes[i].code.equals(s)) continue;
            code = codes[i];
        }
        return code;
    }
