...
public class StringUtils {
    ...
    public static final byte[] getBytes(char[] c, String encoding, String serverEncoding, boolean parserKnowsUnicode, Connection conn) throws SQLException {
        try {
            SingleByteCharsetConverter converter = null;
            converter = conn != null ? conn.getCharsetConverter(encoding) : SingleByteCharsetConverter.getInstance(encoding, null);
    ...
    public static final byte[] getBytes(String s, String encoding, String serverEncoding, boolean parserKnowsUnicode, Connection conn) throws SQLException {
        try {
            SingleByteCharsetConverter converter = null;
            converter = conn != null ? conn.getCharsetConverter(encoding) : SingleByteCharsetConverter.getInstance(encoding, null);
    ...
    public static int indexOfIgnoreCaseRespectMarker(int startAt, String src, String target, String marker, String markerCloses, boolean allowBackslashEscapes) {
        ...
        for (int i = startAt; i < srcLength; ++i) {
            ...
            if (markerTypeFound != -1 && c == markerCloses.charAt(markerTypeFound) && !escaped) {
                contextMarker = '\u0000';
                markerTypeFound = -1;
                continue;
            }
